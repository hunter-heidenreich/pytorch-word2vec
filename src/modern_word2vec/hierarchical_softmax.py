...
class HuffmanTree:
    ...
    def __init__(self, word_counts: Dict[str, int], word_to_idx: Dict[str, int]):
        ...
        self.word_codes, self.word_paths, self.num_inner_nodes = self._build_tree(
            word_counts
        )
    ...
    def _build_tree(
        ...
        next_inner_node_id = (
            vocab_size  # Internal nodes get IDs starting from vocab_size
        )
    ...
    def _generate_codes(
        self,
        node_data,
        current_code: List[int],
        current_path: List[int],
        word_codes: Dict[int, List[int]],
        word_paths: Dict[int, List[int]],
        vocab_size: int,
    ):
...
class HierarchicalSoftmax(nn.Module):
    ...
    def __init__(
        ...
        self.inner_node_embeddings = nn.Embedding(
            self.huffman_tree.num_inner_nodes, embedding_dim
        )
    ...
    def _precompute_paths(self):
        ...
        max_path_length = (
            max(len(path) for path in self.huffman_tree.word_paths.values())
            if self.huffman_tree.word_paths
            else 0
        )

        # Create padded tensors for all word paths and codes
        self.word_path_indices = torch.full(
            (self.vocab_size, max_path_length), -1, dtype=torch.long
        )
        self.word_codes_tensor = torch.full(
            (self.vocab_size, max_path_length), -1, dtype=torch.long
        )
        ...
        for word_idx in range(self.vocab_size):
            if word_idx in self.huffman_tree.word_paths:
                ...
                if path_len > 0:
                    ...
                    self.word_path_indices[word_idx, :path_len] = torch.tensor(
                        path_indices
                    )
                    self.word_codes_tensor[word_idx, :path_len] = torch.tensor(code)

    def forward(
        self, input_embeddings: torch.Tensor, target_words: torch.Tensor
    ) -> torch.Tensor:
        ...
        batch_paths = self.word_path_indices[
            target_words, :max_path_length
        ]  # (batch_size, max_path_len)
        batch_codes = self.word_codes_tensor[
            target_words, :max_path_length
        ]  # (batch_size, max_path_len)

        # Create mask for valid path positions
        path_mask = torch.arange(max_path_length, device=device).unsqueeze(
            0
        ) < batch_path_lengths.unsqueeze(1)
        # (batch_size, max_path_len)
        ...
        inner_embeds_flat = self.inner_node_embeddings(
            safe_paths
        )  # (total_valid_positions, embedding_dim)

        # Reshape back to batch format
        inner_embeds = torch.zeros(
            batch_size,
            max_path_length,
            self.embedding_dim,
            device=device,
            dtype=inner_embeds_flat.dtype,
        )
        ...
        dots = torch.sum(
            input_embeddings.unsqueeze(1) * inner_embeds, dim=2
        )  # (batch_size, max_path_len)

        # Apply sigmoid with correct signs based on codes
        # For code=1, we want sigmoid(dot), for code=0, we want sigmoid(-dot)
        adjusted_dots = dots * (
            2 * batch_codes.float() - 1
        )  # (batch_size, max_path_len)

        # Apply mask to ignore invalid positions
        adjusted_dots = torch.where(
            path_mask, adjusted_dots, torch.zeros_like(adjusted_dots)
        )
        ...
        masked_log_probs = torch.where(
            path_mask, log_probs, torch.zeros_like(log_probs)
        )
    ...
    def predict_probabilities(self, input_embeddings: torch.Tensor) -> torch.Tensor:
        ...
        for word_idx in range(self.vocab_size):
            ...
            inner_embeds = self.inner_node_embeddings(
                path_indices
            )  # (path_length, embedding_dim)
